<x-app-layout title="Articulos">
    <div class="container grid px-6 mx-auto">
        ...
        <a href="{{ route('imagencarousel.create') }}">
            <button type="button"
                        ...
                        <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($carousel as $item)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <img src="{{ $item->ImagenMovil }}" alt="{{ $item->ImagenMovil }}" >
                                    </td>
                                    <td class="px-4 py-3 text-sm">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            <a href="{{route('imagencarousel.edit', $item->id)}}">
                                                <button
                                            ...
                                            <form action="{{route('imagencarousel.destroy', $item->id)}}" method="post" onsubmit="return confirm('Seguro desea eliminar el articulo? Perdera todos los archivos en ella')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
