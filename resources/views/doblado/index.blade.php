<x-app-layout title="Audios Doblados">
    <div class="container grid px-6 mx-auto">
        <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Listar Audios Doblados del  Capitulo {{$capitulo->Nombre}} del anime {{$capitulo->temporada->anime->Titulo}}
        </h2>
        @if (session('success'))
        ...
        <a href="{{ route('doblado.crear', $capitulo->id) }}">
            <button type="button"
                                ...
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex items-center space-x-4 text-sm">
                                            <a href="{{route('doblado.edit', $item->id)}}">
                                                <button
                                            ...
                                            <form action="{{route('doblado.destroy', $item->id)}}" method="post" onsubmit="return confirm('Seguro desea eliminar el articulo? Perdera todos los archivos en ella')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
