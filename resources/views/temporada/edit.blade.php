<x-app-layout title="Actualizar Temporada">
    <div class="container grid px-6 mx-auto">
        <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">
            Actualizar Temporada {{$temporada->Nombre}} de {{$anime->Titulo}}
        </h2>
        <form action="{{route('temporada.update', $anime->id)}}" method="post">
            @csrf
            {{ method_field('PUT') }}
            ...
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <label class="block text-sm">
                    ...
                    <input
                        ...
                        placeholder="Ingrese el nombre de la temporada" id="Nombre" name="Nombre" value="{{$temporada->Nombre}}">
                ...
                <label class="block text-sm">
                    ...
                    <input
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                        placeholder="Ingrese la fecha YYYY-MM-DD " id="Nombre" name="FechaInicio" value="{{$temporada->FechaInicio}}">
                        <x-jet-input-error for="FechaInicio" />
                </label>

                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">Fecha Final</span>
                    <input
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                        placeholder="Ingrese la fecha YYYY-MM-DD final" id="Nombre" name="FechaFin" value="{{$temporada->FechaFin}}">
                        <x-jet-input-error for="FechaFin" />
                </label>

                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">Capitulos</span>
                    <input
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                        placeholder="Ingrese cantidad de capitulos" id="Nombre" name="CantidadCapitulos" value="{{$temporada->CantidadCapitulos}}">
                        <x-jet-input-error for="CantidadCapitulos" />
                </label>
